...
async function verifyProof(input, folderPath) {
  try {
    const rpcUrl = "http://localhost:3347";
    ...
    const snarkjsPath = "./node_modules/.bin/snarkjs";

    // Step 2: Generate proof using fullprove
    console.log("📦 Generating proof using fullprove...");
    execSync(
      `${snarkjsPath} groth16 fullprove "${inputJsonPath}" "${wasmPath}" "${zkeyPath}" "${proofPath}" "${publicPath}"`,
      { stdio: "inherit" }
    );
