...
async function deployWithBridge(folderPath, privateKey, options = {}) {
    ...
    l1web3.eth.accounts.wallet.add(account);
    l1web3.eth.defaultAccount = account.address;
    const l1contract = new l1web3.eth.Contract(l1abi);
    const l1deploytx = l1contract.deploy({ data: '0x' + l1bytecode });
    const l1gasEstimate = await l1deploytx.estimateGas({ from: account.address });
    const l1deployedContract = await l1deploytx.send({from: account.address, gas: l1gasEstimate});
    ...
    generateRelayer(folderPath, l2deployedContract.options.address, l1deployedContract.options.address, abi, l1abi, privateKey);
}
