...
async function verifyAndEmit(input, folderPath, privateKey) {
  try {
    const rpcUrl = "http://localhost:8547"; //Arbitrum Sepolia RPC URL
...
const pi_c = [proof.pi_c[0], proof.pi_c[1]];
    ...
    const result = await contract.methods
      .verifyAndEmit(pi_a, pi_b, pi_c, publicSignals)
      .send({
        from: account.address,
        gas: 3_500_000
      });
      console.log("Transaction hash:", result.transactionHash);
    console.log("verify and emit");
