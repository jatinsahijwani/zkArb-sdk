...
async function compileCircuit(circuitPath) {
    ...
    const wrapperPath = path.join(outDir, 'VerifierWrapper.sol');
...
contract VerifierWrapper is Groth16Verifier {
    ...
    const snarkjsBin = path.resolve(__dirname, "../node_modules/.bin/snarkjs");
    
  
    // Run groth16 setup
    execSync(
        `"${snarkjsBin}" groth16 setup "${r1csPath}" "${ptauPath}" "${zkeyPath}"`,
        { stdio: "inherit" }
    );
    
    // Export verifier.sol solidity contract
     execSync(
    `"${snarkjsBin}" zkey export solidityverifier "${outDir}/circuit_final.zkey" "${outDir}/verifier.sol"`,
    { stdio: "inherit" }
  );
